...
public class MMOData extends AbstractData<MMOData, ImmutableMMOData> {
	...
	public MMOData() { this(Maps.newHashMap(), Maps.newHashMap(), true, true, false); }
	public MMOData(@Nonnull final Map<String, Long> delays, @Nonnull final Map<String, Long> abilities, final boolean action_bar, final boolean scoreboard, final boolean scoreboard_permanent) {
		this.delays = delays;
	...
	public Value<Map<String, Long>> getAbilitiesValue() { return MMOKeys.FACTORY.createMapValue(MMOKeys.ABILITIES, this.abilities); }

	public Value<Boolean> getActionBarValue() { return MMOKeys.FACTORY.createValue(MMOKeys.ACTION_BAR, this.action_bar); }
	...
	public long getDelay(@Nonnull final String id) { return Optional.ofNullable(this.delays.get(id)).orElse(0L); }
	public void setDelays(Map<String, Long> delays) { this.delays = delays; }
	public void setDelay(@Nonnull final String id, final long endtime) { this.delays.put(id, endtime); }
	public boolean isDelayActive(@Nonnull final String id) { return this.getDelay(id) > System.currentTimeMillis(); }

	public Map<String, Long> getAbilities() { return this.abilities; }
	public long getAbility(@Nonnull final String id) { return Optional.ofNullable(this.abilities.get(id)).orElse(0L); }
	public void setAbilities(@Nonnull final Map<String, Long> abilities) { this.abilities = abilities; }
	public void setAbility(@Nonnull final String id, final long endtime) { this.abilities.put(id, endtime); }
	public boolean isAbilityActive(@Nonnull final String id) { return this.getAbility(id) > System.currentTimeMillis(); }

	public boolean getActionBar() { return this.action_bar; }
	public void setActionBar(final boolean action_bar) { this.action_bar = action_bar; }
	...
	public Optional<MMOData> from(@Nonnull final DataView view) {
		return Optional.of(new MMOData(
				view.getMap(MMOKeys.DELAYS.getQuery()).map(map -> Maps.newHashMap((Map<String, Long>) map)).orElse(Maps.newHashMap()),
				view.getMap(MMOKeys.ABILITIES.getQuery()).map(map -> Maps.newHashMap((Map<String, Long>) map)).orElse(Maps.newHashMap()),
	...
	public DataContainer toContainer() {
		return super.toContainer()
				.set(MMOKeys.DELAYS.getQuery(), this.delays)
				.set(MMOKeys.ABILITIES.getQuery(), this.abilities)
				.set(MMOKeys.ACTION_BAR.getQuery(), this.action_bar)
				.set(MMOKeys.SCOREBOARD.getQuery(), this.scoreboard)
				.set(MMOKeys.SCOREBOARD_PERMANENT.getQuery(), this.scoreboard_permanent);
	}

	@Override public Optional<MMOData> fill(DataHolder holder, MergeFunction overlap) { return Optional.of(Preconditions.checkNotNull(overlap).merge(copy(), from(holder.toContainer()).orElse(null))); }
	@Override public Optional<MMOData> from(DataContainer container) { return from((DataView)container); }
	@Override public MMOData copy() { return new MMOData(this.delays, this.abilities, this.action_bar, this.scoreboard, this.scoreboard_permanent); }
	@Override public ImmutableMMOData asImmutable() { return new ImmutableMMOData(this.delays, this.abilities, this.action_bar, this.scoreboard, this.scoreboard_permanent); }
