...
public class ScoreboardManager extends MMOObject {
	...
	public void setTemp(@Nonnull final Player p, @Nonnull final Text title, @Nonnull final Multimap<Integer, Text> lines) {
		...
		this.tasks.put(p.getUniqueId(), Task.builder().delayTicks(300).execute(() -> this.unload(p)).submit(super.getMMO()));
	}
...
//		Objective obj = Objective.builder().name("MMO_" + (p.getName().length() > 12 ? p.getName().substring(0, 12) : p.getName())).criterion(Criteria.DUMMY).displayName(title).build();
	...
	public void unload(@Nonnull final Player p) {
		Optional.ofNullable(this.tasks.get(p.getUniqueId())).ifPresent(t -> { t.cancel(); this.tasks.remove(p.getUniqueId()); });
