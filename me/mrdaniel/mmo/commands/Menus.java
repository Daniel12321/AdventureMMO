...
public class Menus {
	...
	public static void sendSkillInfo(Player p, MMOPlayer mmop, SkillType type, Skill skill, AbilityEnum ae, PassiveEnum... pe) {
		p.sendMessage(Text.of(""));
		...
		if (ae != null) { 
			int millis = (int) ((((double)skill.level)/0.02) + 5000.0);
			double seconds = ((double)millis) / 1000;
			p.sendMessage(Text.of(""));
			p.sendMessage(Text.of(TextColors.RED, "-= ", TextColors.YELLOW, ae.name, TextColors.RED, " =-"));
			p.sendMessage(Text.of(TextColors.DARK_GREEN, ae.desc));
			p.sendMessage(Text.of(TextColors.DARK_GREEN, "Duration: ", String.valueOf(seconds), "s"));
		}
		for (PassiveEnum pes : pe) {
			p.sendMessage(Text.of(""));
			p.sendMessage(Text.of(TextColors.RED, "-= ", TextColors.YELLOW, pes.name, TextColors.RED, " =-"));
			p.sendMessage(Text.of(TextColors.DARK_GREEN, pes.desc));
			p.sendMessage(Text.of(TextColors.DARK_GREEN, "Chance: ", String.valueOf(pes.start + (pes.increase * ((double)skill.level))), "%"));
		}
	}
