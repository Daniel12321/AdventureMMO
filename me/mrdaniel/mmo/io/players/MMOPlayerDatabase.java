...
public class MMOPlayerDatabase {
	...
	public void store(MMOPlayer mmop) {
		...
		catch (Exception exc) { Main.getInstance().getGame().getServer().getConsole().sendMessage(Text.of(TextColors.RED, "[SpongeMMO]: Error saving player file")); exc.printStackTrace(); }
	}
	private SkillSet read(File file) {
		...
		catch (Exception exc) { Main.getInstance().getGame().getServer().getConsole().sendMessage(Text.of(TextColors.RED, "[SpongeMMO]: Error loading player file")); return SkillSet.getEmpty(); }
	}
}