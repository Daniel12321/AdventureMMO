...
public class ChunkManager {
	...
	public void write(List<Coord> store, File file) {
		...
		catch (Exception exc) { Main.getInstance().getGame().getServer().getConsole().sendMessage(Text.of(TextColors.RED, "[SpongeMMO]: Error saving chunk file")); exc.printStackTrace(); }
	}
	public List<Coord> read(File file) {
		...
		catch (Exception exc) { Main.getInstance().getGame().getServer().getConsole().sendMessage(Text.of(TextColors.RED, "[SpongeMMO]: Error loading chunk file")); return new ArrayList<Coord>(); }
	}
}